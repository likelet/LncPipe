...
if (params.help) {
    ...
    log.info '    The typical command for running the pipeline is as follows:\n' +
            ...
            print_cyan('      --hisat_index                  ')+print_green('Path to hisat index(required)\n') +
...
params.hisat_idex = '/data/database/hg38/GENCODE/hisatIndex'
...
log.info print_yellow("hisat index path               :")+ print_green(params.hisat_idex)
...
hisat_idex = file(params.hisat_idex)
...
//hisat index
//hisat_ref = file(params.params.hisat_idex_ref)
...
// Check whether fastq file is available
    ...
    if (params.mode == 'fastq') {
        ...
        println "Start mapping with hisat aligner"
//
...
// Match the pairs on two channels
        ...
        //run hisat iterate over sample or parallel
        paramode = true
// hisat alignment
        if (paramode) {
            process fastq_hisat_alignment_para {
                cpus params.cpu
                tag { file_tag }
                maxForks 1
                ...
                output:
                set val(file_tag_new), file("hisat_${file_tag_new}") into hisatmappedReads,alignment_logs
//                file("hisat_${file_tag_new}/*final.out")into alignment_logs

                shell:
                file_tag = pair[0].name.replace("${params.suffix1}.${params.fastq_ext}", "")
                file_tag_new = file_tag
                hisat_threads = params.cpu.intdiv(2) - 1
...
/* publishDir params.out_folder, mode: 'move', overwrite: true*/
                ...
                output:
                set val(file_tag_new), file("hisat_${file_tag_new}") into hisatmappedReads,alignment_logs
//                file "!{file_tag_new}_hisat_log.txt" into alignment_logs
                shell:
                file_tag = pair[0].name.replace("${params.suffix1}.${params.fastq_ext}", "")
                file_tag_new = file_tag
                hisat_threads = params.cpu.intdiv(2) - 1
...
// cufflinks
        process cufflinks_assembly {
            ...
            input:
            set val(file_tag), file(hisat_alignment) from hisatmappedReads
            file fasta_ref
            file gencode_annotation_gtf
            file rRNAmaskfile
            ...
            -o Cufout_!{file_tag_new} -p 25 !{hisat_alignment}/!{file_tag_new}Aligned.sortedByCoord.out.bam
